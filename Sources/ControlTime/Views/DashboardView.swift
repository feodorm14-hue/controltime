...
struct ShortcutsGuideSheet: View {
    ...
    private var steps: some View {
        VStack(alignment: .leading, spacing: 12) {
            step("1", "Открой «Быстрые команды» → вкладка «Автоматизация» → «+»")
            step("2", "Выбери «Приложение» → выбери нужное приложение → «При открытии» → «Далее»")
            step("3", "«Новая пустая автоматизация» → «+»")
            step("4", "Найди «ControlTime» → добавь «Упражнение выполнено?»")
            step("5", "Добавь «Если» (If) → значение результата = НЕТ (false)")
            step("6", "Внутри «Если» добавь ControlTime → «Начать упражнение»")
            step("7", "Сохрани, сними галку «Спрашивать перед запуском»")

            Text("Логика: если упражнение уже выполнено — пускает в приложение. Если нет — открывает ControlTime.")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .padding(.top, 4)
        }
